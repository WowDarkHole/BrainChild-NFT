...
const ReactPageScroller = ({
  ...
  const [nextComponentIndex, setNextComponentIndex] = useState(0);
  ...
  const scrollPage = useCallback(
    nextComponentIndex => {
      ...
      setNextComponentIndex(nextComponentIndex);
  ...
  useEffect(() => {
    ...
    [customPageNumber, animationTimer, animationTimerBuffer, children, componentsToRenderLength, scrollPage]
  );
  ...
  return (
    <>
      ...
      <Scrollbar className="fixed bottom-20 left-16 hidden sm:block" slide={nextComponentIndex}/>
      <Logo className="top-0 left-1/2" slide={nextComponentIndex} ref={{containerRef: containerRefs[nextComponentIndex], contentRef: contentRefs[nextComponentIndex]}}/>
    </>
  );
};
